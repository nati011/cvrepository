    ...
    <div className="flex flex-col gap-8">
      <nav className="text-sm" aria-label="Breadcrumb">
        <ol className="flex flex-wrap items-center gap-2 text-slate-500">
          ...
          <li className="truncate text-slate-700">
            {cv.title || cv.original_filename}
          </li>
        </ol>
      </nav>

      <div className="flex flex-col gap-6 border-b border-slate-200/90 pb-8 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">
            {cv.title || cv.original_filename}
          </h1>
          <p className="mt-2 text-sm text-slate-500">{cv.original_filename}</p>
      ...
      <section className="rounded-xl border border-slate-200/90 bg-white p-6 shadow-card">
        ...
        <iframe
          ...
          src={`/api/cvs/${id}/file`}
          className="mt-4 h-[min(80vh,900px)] w-full rounded-lg border border-slate-200"
        />
      </section>
    </div>
  );
